...
module.exports = React.createClass({
    ...
    loginLinL:function(options){ 
        var _this=this;
        $.ajax({
           type: "POST",
           url:WebIM.config.loginUrl,
           data: {mobile:options.user,passwd:options.pwd},
           success: function(msg){
             if(msg.status==1){
                    var opt = {
                        apiUrl: _this.props.config.apiURL,
                        user:msg.data.auths.emcaht.username.toLowerCase(),
                        pwd: msg.data.auths.emcaht.password,
                        accessToken:msg.data.auths.emcaht.password,
                        appKey: _this.props.config.appkey
                    };
                    console.log("账户："+opt.user+"__密码"+opt.pwd);
                new Store("token").create({id:"userToken",token:msg.data.authorization.value});
                _this.loginHx(opt);
             }else{
                     Demo.api.NotifyError(msg.data.remart);
             } 
           }, 
           error:function(e){
             Demo.api.NotifyError(e);
           }
         }); 
    },
    loginHx:function(options){
        var type = this.refs.token.refs.input.checked;
         //判断使用token登录
        if (!type) {
            delete options.accessToken;
        } 
        Demo.user = options.username 
