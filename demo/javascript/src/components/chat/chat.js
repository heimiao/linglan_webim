...
module.exports = React.createClass({
    ...
    getInitialState: function () {
        ...
        if(WebIM.config.isLinglan){
            this.mapName(WebIM.config.getDoctor);
            this.mapName(WebIM.config.getAssistant);
            this.mapName(WebIM.config.getPatient);
        }
    ...
    //名字映射请求
    mapName:function(url){
    var token=_.values(new Store("token").findAll())[0].token; 
         $.ajax({
           type: "GET",
            headers:{authorization:token},
            beforeSend: function(xhr) {   
             xhr.setRequestHeader("Authorization", "as " + token);  
            },
           url:url, 
           success: function(msg){
              ...
              if(msg.status==1){
                ...
                for(var key in myData){
                    Demo.mapName[myData[key].userId]=myData[key].userName;
                } 
              }else{
                 // Demo.api.NotifyError(msg.err);
    ...
    getLingLanChat:function(args){ 
        var me=this,rooms=[],datas= _.values(new Store("token").findAll())[0].token;
         $.ajax({
               type: "GET",
               headers:{authorization:datas},
                beforeSend: function(xhr) {   
                 xhr.setRequestHeader("Authorization", "as " + datas);  
                },
               url:WebIM.config.sessionAssistant, 
               success: function(msg){ 
                  if(msg.status==1){
                    var data=msg.data;
                    data.forEach((m, n) => {
                        ...
                        for(var i=0;i<args.length;i++){
                            if(m.groupId==args[i].roomId){
                                $.extend(result,m,args[i])
                            } 
                        }
                        rooms.push(result);
                    })          
    ...
    storeChatWindow: function(){
        var id, cate = '',
            props = {
                sendScreenshots:this.sendScreenshots,
    ...
    //发送截图
    sendScreenshots:function(chatType){
    }, 
    ...
    render: function () {
        ...
        return (
            <div className={this.props.show ? 'webim-chat' : 'webim-chat hide'}>
                  ...
                  <LeftBar cur={this.state.cur} update={this.update} style="{diplay:none}"/>
