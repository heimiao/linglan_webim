...
module.exports = {
    ...
    init: function () {
        ...
        Demo.strangers = {};
        Demo.blacklist = {};
        Demo.selectedCate = 'groups';//friends strangers chatrooms groups
    ...
    appendMsg: function (msg, type) {
        ...
        var brief = '',
          ...
          //var ary={backbone:"backbone框架",redux:"redux框架",vue:"vue框架",angular:"angular框架",ng:"angular框架",react:"react框架",test:"测试",datang1:"大唐1"};
         var ary=Demo.mapName;
         $.each(ary,function(n,m){
            if(n==name){
                name=m;
            }
         });
        ...
        switch (msg.type) {
            case 'chat':
                ...
                this.addCount(msg.from, cate,msg);
                break;
            case 'groupchat':
                cate = msg.roomtype ? msg.roomtype : 'groups';
                this.addCount(msg.to, cate,msg);
                break;
        }
    },
    ...
    addCount: function (id, cate,msg) {
        ...
        if (Demo.selectedCate !== cate) {
            ...
            if (!this.sentByMe && id !== Demo.selected) { 
                var slectedItem=document.getElementById(id);
                var cur = slectedItem.getElementsByTagName('i')[0]; 
                var curCount = cur.getAttribute('data-count') / 1;
                curCount++;
                cur.setAttribute('data-count', curCount);
                Demo.chatRecord[id].count = curCount;
                cur.innerText = curCount > 999 ? '...' : curCount + '';
                cur.style.display = 'block';
                if(WebIM.config.isLinglan)this.noticeSelf(id,msg);

            }
        }

    }, 
    noticeSelf:function(ID,msg){
        //如果群里有@的自己的调用该方法noticeColor
        //{"id":"285141589406255152","type":"groupchat",
        //"from":"backbone","to":"1482837779060","data":"@ng",
        //"ext":{"em_at_list":["ng"]},"error":false,"errorText":"",
        //"errorCode":""}
        //判断页面是否有该属性，然后判断该信息中是否包含@自己如果包
       var spanDom=document.getElementById(ID).getElementsByTagName("div")[1].querySelector("span");
       var bool= spanDom.className.split(" ").some(function(item){
            //判断之前是否有被@自己的
            return (item == "noticeColor");
        }) 
        if(msg.ext.em_at_list){
            msg.ext.em_at_list.forEach(function(item){
                if(item==Demo.user){
                    if(!bool){
                         spanDom.className+=" noticeColor"; 
                    }
                } 
            })
        }  
